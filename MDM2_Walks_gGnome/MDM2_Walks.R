...
A1KU_Ggraph <-  gG(jabba="Jabba_Calls_TCGA/A1KU/jabba.simple.rds")
...
jabbaX <- gG(jabba="Jabba_Calls_TCGA/A1KU/jabba.simple.rds")
CNmax <- max(na.omit(jabbaX$dt$cn))                                                                                                                                                     
CNxt = CNmax *0.5                                                                                                                                                                       
while (CNxt >=8) {                                                                                                                                                                      
  print(CNxt)
  highcopyX = jabbaX[cn>CNxt]        # subset to high copy only                                                                                                                                                   
  walks = highcopyX$walks()                                                                                                                                                           
  saveRDS(walks, paste0('MDM2_Walks_Out/A1KU/walks_CNmin', CNxt,'.rds')) 
  print('Walks done')
  walks_circ <- walks[circular == T]            # ecDNA?                                                                                                                                      
  if (length(walks_circ) > 0) {
      print('Found a circular amplicon')
      saveRDS(walks_circ, paste0('MDM2_Walks_Out/A1KU/walksCirc_CNmin', CNxt,'.rds'))                                                                                                 
      ...
      pdf(file = paste0('MDM2_Walks_Out/A1KU/walksCirc_CNmin', CNxt,'.pdf'))                                                                                       
      plot(c(gencode, jabbaX$gt, walks_l$gtrack(name = "circular walks")), walks_l$footprint+2e5, cex.label=0.2)
