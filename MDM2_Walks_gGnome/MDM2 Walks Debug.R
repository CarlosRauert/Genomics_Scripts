...
lleft = which(highcopyX$nodes$loose.left)
lright = which(highcopyX$nodes$loose.right)

llefti = highcopyX$queryLookup(lleft)$index
lrighti = highcopyX$queryLookup(lright)$index

lleftir = highcopyX$queryLookup(-lleft)$index
lrightir = highcopyX$queryLookup(-lright)$index

sources = c(llefti, lrightir)
sinks = c(lrighti, lleftir)

ap = all.paths(A, sources = sources, sinks = sinks, verbose = TRUE)
...
sparse_subset = function(A, B, strict = FALSE, chunksize = 100, quiet = FALSE)
{
  ...
  for (i in seq(1, max(1,nrow(A)), chunksize))
  {
